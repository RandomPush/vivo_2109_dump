...
function zwb_create_file_normal() {
...
function zwb_create_file_ko() {
	BDRealPath="/data/vendor/swap/zram"
	BDLoopPath=`losetup -f`
	BDPath=$BDLoopPath
	if [ "$ZWBTriggerUser" -eq "0" ]; then
		rm $BDRealPath
		return
	...
	FileSizeB=`stat -c "%s" $BDRealPath`
	BDSizeB=`expr $BDSizeMB \* 1048576`
	if [ "$BDSizeB" == "$FileSizeB" ] || ! zwb_storage_check ; then
		losetup $BDLoopPath $BDRealPath
		return
	fi
	...
	is_f2fs2=`mount -r -t f2fs | grep " /data "`
	if [ "$is_f2fs1" == "" ] && [ "$is_f2fs2" == "" ]; then
		dd if=/dev/zero of=$BDRealPath bs=1m count=$BDSizeMB
	else
		touch $BDRealPath
		f2fs_io pinfile set $BDRealPath
		fallocate -l $BDSizeB -o 0 $BDRealPath
	fi
	losetup $BDLoopPath $BDRealPath
}

function zwb_create_file() {
	zram_ko=`lsmod | grep zram`
	if [ "$zram_ko" == "" ]; then
		zwb_create_file_normal
	else
		zwb_create_file_ko
	fi
}
